	  ...
	  public:
		...
		*/
		 ...
		 */
		 ...
		 /*Quita una informacion asocida a una clave que está en el diccionario.
			Si no esta la clave o la definicion devuelve false.
		 */
		bool removeSignificadoPalabra(const T & clave, const U &definicion){
			typename list<data<T,U> >::iterator it;
			...
			for (typename list<U>::iterator itdef = (*it).info_asoci.begin(); itdef != (*it).info_asoci.end(); ++it)
				if (definicion == (*itdef))
				{
					(*it).info_asoci.erase(itdef);
					return true;
				}
		...
		/* Devuelve un diccionario con toda la información entre dos claves.
		 * Es precondición que la clavea se encuentre antes en el diccionario que la claveb*/
		Diccionario getDiccionarioEntreClaves(const T & clavea, const T & claveb)
		{
			Diccionario<T,T> D;
			typename list<data<T,U>>::iterator ita, itb;
			
			if (Esta_Clave(clavea, ita))
				if (Esta_Clave(claveb, itb))
				{
		...
		bool cambiarClave (const T & claveAntigua, const T & claveNueva)
		{
			typename list<data<T,U>>::iterator it;
			
			if (Esta_Clave(claveAntigua, it))
			{
				(*it).clave = claveNueva;
				return true;
			}
